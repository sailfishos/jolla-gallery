...
TestCase {
    ...
    function dump_tree(item, indent) {
        ...
        if (item.hasOwnProperty("text")) {
            if (item.text.length > 10) {
                desc += " text: '" + item.text.substr(0, 27) + "...'"
            } else {
                desc += " text: '" + item.text + "'"
            }
        }
    ...
    function pull_up(drag) {
        ...
        // touch point
        var touch_point = real_height * 0.5 - 10

        var steps = drag/drag_y
        var step = 1
//        while (step < steps) {
            step += 1

            var drag_end = touch_point
            var pos = main.mapFromItem(page, drag_x, drag_end)
            var mp = testEvent.mousePress(main, pos.x, drag_end, Qt.LeftButton, 0, 0)
            console.log("\n\n pos.x:" + pos.x+ "\n\n")
            console.log("\n\n steps:" + steps + "\n\n")
            console.log("\n\n mp:" + mp + "\n\n")
            ...
            while (drag_y < drag_end) {
                ...
                pos = main.mapFromItem(page, drag_x, drag_end)
                testEvent.mouseMove(main, pos.x, pos.y, Qt.LeftButton, 0, 0)
                wait(1)
            }
//        }
        ...
        testEvent.mouseRelease(main, pos.x, pos.y, Qt.LeftButton, 0, 0)
        //        fail("Could not pull page")
    }


    function pull_down(direction) {
        ...
        var pos = main.mapFromItem(page, drag_x, drag_y)
        console.log("\n\n pos.x:" + pos.x+ "\n\n")
        var mp = testEvent.mousePress(main, pos.x, 50, Qt.LeftButton, 0, 0)
        console.log("\n\n mp:" + mp + "\n\n")
        while (drag_y < drag_end) {
            drag_y += 10
            //            console.log("\n\n drag_y:" + drag_y + "\n\n")
            pos = main.mapFromItem(page, drag_x, drag_y)
            var mm = testEvent.mouseMove(main, pos.x, pos.y, Qt.LeftButton, 0, 0)
            console.log("\n\n pos.x:" + pos.x+ " ")
            console.log("pos.y:" + pos.y+ "\n\n")
            console.log("\n\n mm:" + mm + "\n\n")
            wait(1)
